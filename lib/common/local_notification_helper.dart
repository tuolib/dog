...
class NotificationHelper {
  void configureSelectNotificationSubject() {
    selectNotificationSubject.stream.listen((String payload) async {
      print('_configureSelectNotificationSubject: $payload');
...
//      );
  ...
  Future<void> showNotification(Map<String, dynamic> channelInfo) async {
    var androidPlatformChannelSpecifics = AndroidNotificationDetails(
        channelInfo['id'].toString(), channelInfo['content'], channelInfo['content'],
        importance: Importance.Max, priority: Priority.High, ticker: 'ticker');
    var iOSPlatformChannelSpecifics = IOSNotificationDetails();
    var platformChannelSpecifics = NotificationDetails(
        androidPlatformChannelSpecifics, iOSPlatformChannelSpecifics);
    var payloadStr = json.encode(channelInfo);
    print('channelInfo: $channelInfo');
    var title = channelInfo['username'];
    if (channelInfo['groupType'] == 1) {
      title = '${channelInfo['username']}[${channelInfo['groupName']}]';
    }
    await flutterLocalNotificationsPlugin.show(
        ...
        payload: payloadStr);
// 如果app 在打开【foreground】的情况下，通知显示3秒
    print('AppLifecycleState: ${AppLifecycleState.inactive}');
    print('lifecycleState: $lifecycleState');
    if (lifecycleState == AppLifecycleState.inactive) {
      Timer(Duration(seconds: 3), () {
        flutterLocalNotificationsPlugin.cancel(channelInfo['id']);
      });
    }
  }
